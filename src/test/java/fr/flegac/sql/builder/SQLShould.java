...
public class SQLShould {
  ...
  public void buildSQLRequest() {
    // given
    final String expected = "SELECT * FROM documentType";

    // when
    final String query = selectAllFields().from("documentType").build();
  ...
  public void buildSQLWithMultipleWhereClause() {
    // given
    final String expected = "SELECT * FROM database WHERE ((clause1 AND clause2) AND (choice1 OR choice2 OR choice3))";

    // when
    final String query = selectAllFields().from("database")
        .where("clause1")
        .where("clause2")
  ...
  public void buildSQLWithOrderByWithoutWhereClause() {
    // given
    final String expected = "SELECT * FROM dataBase WHERE 1=1 ORDER BY X, Y ASC, Z DESC";
  ...
  public void buildSQLWithWhereClause() {
    // given
    final String pattern1 = "%SUFFIX";
    final String pattern2 = "PREFIX%";
    final String expected = "SELECT field1, field2 FROM documentType"
        + " WHERE ('A' = B"
    ...
    // when
    final String query = select("field1, field2").from("documentType")
        .where(allTrue(
            "'A' = B",
